...
exports.ConstantCurveCalculator = exports.CurveCalculator = exports.TradeDirection = void 0;
const anchor_1 = require("@coral-xyz/anchor");
const bn_1 = require("@hashfund/bn");
var TradeDirection;
...
class CurveCalculator {
    static calculateAmountOut(virtualTokenReserve, virtualPairReserve, amount, tradeDirection) {
        throw new Error("method not implemented");
    }
}
exports.CurveCalculator = CurveCalculator;
class ConstantCurveCalculator {
    rawSupply;
    liquidityPercentage;
    decimals;
    virtualTokenSupply;
    vp_0;
    constructor(rawSupply, liquidityPercentage, decimals = 6) {
    ...
    get virtualTokenReserve() {
        ...
        return (0, bn_1.unsafeBN)(new anchor_1.BN(BigInt(Math.round(val * (10 ** this.decimals))).toString()));
    }
    get tokenBReserveBalance() {
        const val = Number.isNaN(this.vp_0) ? 0 : this.vp_0;
        return (0, bn_1.unsafeBN)(new anchor_1.BN(BigInt(Math.round(val * 1e9)).toString()));
    }
    get bondingCurveSupply() {
        return this.rawSupply.mul(new anchor_1.BN(this.liquidityPercentage)).div(new anchor_1.BN(100));
    }
    ...
    calculateInitialPrice() {
        if (this.virtualTokenSupply === 0)
            return 0;
        return this.vp_0 / this.virtualTokenSupply;
    }
    static calculateAmountOut(virtualTokenReserve, virtualPairReserve, amount, tradeDirection, isMigrated = false) {
        ...
        switch (tradeDirection) {
            ...
            case TradeDirection.AtoB: {
                ...
                return grossSol.mul(new anchor_1.BN(95)).div(new anchor_1.BN(100));
            }
        }
    }
    static calculateAmountOutNumber(virtualTokenReserve, virtualPairReserve, amount, tradeDirection, isMigrated = false) {
        if (amount <= 0 || virtualTokenReserve <= 0 || virtualPairReserve <= 0)
            return 0;
...
exports.ConstantCurveCalculator = ConstantCurveCalculator;
